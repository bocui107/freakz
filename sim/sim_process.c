...
bool sim_check_readpipe()
{
	 ...
	 * the select call scans the fdset bitmap up to the
	 * specified fd number so we need to get the max and
	 * add 1 to it.
	 */
	max_fd = (self_node.data_rx[READPIPE] > self_node.cmd_rx[READPIPE]) ?
		  self_node.data_rx[READPIPE] + 1 : self_node.cmd_rx[READPIPE] + 1;
