...
#define OCRSetup()				\
	 ...
	 * pre-scale factor is 1024,		\
	 * i.e. we have 125 "ticks" / sec:	\
	...
	TIFR = 0x00;				\
	/*					\
	 * Raise interrupt when value in OCR0	\
	 * is reached. Note that the counter	\
	 * value in TCNT0 is cleared		\
	 * automatically.			\
	 */					\
	TIMSK = _BV (OCIE0);
...
/*
 * Chris - changed the settings on the clock from the original.
 * FreakZ performs better with a 1 msec tick interval instead
 * of the original 8 msec interval
 */
#define OCRSetup() \
	/* Select internal clock */		\
	ASSR = 0x00;				\
