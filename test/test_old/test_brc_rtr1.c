...
/*******************************************************************
    ...
    Title: test_brc_rtr1.c

    Description:
    Test the network broadcast function. This test only sends the broadcast
    to routers and coordinators. We send a return broadcast from a router after the 2nd
    attempt, after which, the device should timeout the broadcast and not send the
    third attempt.
*******************************************************************/
#include "contiki.h"
#include "zigbee.h"
...
**************************************************************************/
static void test_brc_rtr_gen_neighbors()
{
    U8 i;
...
static void test_brc_rtr_gen_frame_in(buffer_t *buf, aps_hdr_t *aps_hdr, nwk_hdr_t *nwk_hdr, mac_hdr_t *mac_hdr)
{
    ...
    // change the sequence number so that it won't match seq num from previous broadcast test
    nwk_hdr->seq_num                        = rand();
...
/**************************************************************************
Function Name: test_brc_rtr_gen_headers

Description:
Generate the headers for the buffer.
**************************************************************************/
void test_brc_rtr_gen_headers(buffer_t *buf, aps_hdr_t *aps_hdr, nwk_hdr_t *nwk_hdr, mac_hdr_t *mac_hdr)
{
...
**************************************************************************/
void test_brc_rtr_load_check_data_in()
{
...
Check the final forwarded frame.
**************************************************************************/
void test_brc_rtr_check_data_frm(buffer_t *buf, aps_hdr_t *aps_hdr, nwk_hdr_t *nwk_hdr, mac_hdr_t *mac_hdr)
{
    mac_hdr_t mac_hdr_exp;
    nwk_hdr_t nwk_hdr_exp;
    aps_hdr_t aps_hdr_exp;
...
PT_THREAD(test_brc_rtr1(struct pt *pt))
{
    ...
    test_brc_rtr_gen_headers(tx_buf, &aps_hdr, &nwk_hdr, &mac_hdr);

    // save the seq number for future use
    seq = nwk_hdr.seq_num;
    ...
    DBG_PRINT("TEST_BRC_RTR1: Forwarded broadcast frame received. Sending in response broadcast.\n");

    // generate the test frame and send it into the device
    BUF_ALLOC(tx_buf, TX);
    test_brc_rtr_gen_frame_in(tx_buf, &aps_hdr, &nwk_hdr, &mac_hdr);

    // modify the frame to show that the broadcast is coming from one of the devices that received the broadcast.
    mac_hdr.src_addr.short_addr = 0x1;
    nwk_hdr.src_addr = 0x1;
    nwk_hdr.seq_num = seq;
    test_brc_rtr_gen_headers(tx_buf, &aps_hdr, &nwk_hdr, &mac_hdr);
    ...
    test_driver_rx_isr();
    buf_free(tx_buf);
    DBG_PRINT("TEST_BRC_RTR1: Data Received.\n");

    // wait until the brc timer expires before starting next test
    pcb = nwk_pcb_get();
    PT_WAIT_UNTIL(pt, pcb->brc_accept_new);

    // check to see if any transactions occurred and fail if so.
    if ((test_driver_read_tx_buf(NULL)) != 0) assert(0);
