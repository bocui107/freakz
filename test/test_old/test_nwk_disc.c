...
Load the check data.
**************************************************************************/
void test_load_check_data(U8 channel, mac_hdr_t *hdr)
{
    pan_descr_t desc;
    nwk_nib_t *nib = nwk_nib_get();
    ...
    while (1)
    {
        ...
        if (!data_received)
        {
            ...
            if (test_driver_get_channel() == MAC_MAX_CHANNELS + MAC_PHY_CHANNEL_OFFSET - 1)
            {
                ...
                test_load_check_data(test_driver_get_channel(), &hdr);
            }
        }
        else
        {
